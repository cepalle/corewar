...
static int 	ft_no_oct_params(t_vm *vm, t_proc *processor, int op)
{
	if (gopt()[op].opcode == 1)
	{
		...
		processor->cmd_save.params[0] = vm_read_4(vm, vm->process->PC + 1);
	}
	else
	{
		processor->cmd_save.params_size[0] = 2;
		processor->cmd_save.params[0] = vm_read_2(vm, vm->process->PC + 1);
	}
...
static int 	ft_analyze_oct_params(t_vm *vm, t_proc *processor, int op)
{
	...
	while (i < gopt()[op].nb_arg)
	{
		...
		if (tmp == DIR_CODE && gopt()[op].dir_size_2 == 0)
		{
		...
		if (tmp == IND_CODE || gopt()[op].dir_size_2 == 1)
		{
...
int 		stock_cmd(t_vm *vm, t_proc *processor)
{
	...
	ft_printf("cycle %d\n", processor->cmd_save.cycle_wating);
